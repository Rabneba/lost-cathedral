...
for axis in [0,1]:
 for index in range(-48,49):
  loc=[0,0,.001];loc[axis]=index*.5
  bpy.ops.mesh.primitive_cube_add(size=1,location=loc);line=bpy.context.object;line.dimensions=(.008,48,.003) if axis==0 else (48,.008,.003);line.data.materials.append(lineMat)
...
floor.location.z=0
reviewLights=[(o,o.location.copy())for o in scene.objects if o.type=='LIGHT']
for f in frames:
 rig.matrix_world=rigBase.copy();phase=f%(meta['clips'][clip]['duration']*30);scene.frame_set(int(phase),subframe=phase-int(phase));bpy.context.view_layer.update();rig.matrix_world.translation+=movement*speed*f/30;bpy.context.view_layer.update()
 ...
 center=movement*speed*f/30
 for o,base in reviewLights:o.location=base+center
 cam.location=Vector((center.x,center.y-5,2.0)) if abs(movement.x)>.6 else Vector((center.x+5,center.y,2.0))
 target=center+Vector((0,0,1));cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=3.6
