import bpy,os,sys,math,statistics,json,subprocess,tempfile
...
bpy.ops.object.camera_add();cam=bpy.context.object;scene.camera=cam;cam.data.type='ORTHO';cam.data.ortho_scale=4.4 if clip=='dodge'else 3.7
scene.render.engine='CYCLES';scene.cycles.samples=12;scene.cycles.use_denoising=True;scene.render.resolution_x=900;scene.render.resolution_y=650;scene.render.resolution_percentage=100;scene.view_settings.view_transform='AgX'
os.makedirs(os.path.dirname(ROOT+'/'+out),exist_ok=True);temporary=tempfile.NamedTemporaryFile(prefix='player-'+clip+'-',suffix='.png',delete=False);temporary.close();encoder=subprocess.Popen(['/opt/homebrew/bin/ffmpeg','-hide_banner','-loglevel','error','-f','image2pipe','-framerate',args[5]if len(args)>5 else '30','-i','-','-c:v','libx264','-crf','20','-pix_fmt','yuv420p','-movflags','+faststart',ROOT+'/'+out],stdin=subprocess.PIPE)
manifestPath=ROOT+'/docs/player-combat-revision/'+os.path.basename(blend).replace('.blend','-manifest.json');manifest=json.load(open(manifestPath if os.path.exists(manifestPath)else ROOT+'/docs/player-combat-revision/player-video-candidate-manifest.json'));curve=manifest['clips'].get(clip,{}).get('travelCurve',[]);direction=manifest['clips'].get(clip,{}).get('direction');baseRig=rig.matrix_world.copy()
def distance(t):
 if not curve:return 0
 for (a,x),(b,y)in zip(curve,curve[1:]):
  if a<=t<=b:return x+(y-x)*(t-a)/(b-a)
 return curve[-1][1]if t>curve[-1][0]else 0
autoTarget=None;autoOffset=None
if len(args)>4 and args[4]=='auto':
 # Fit actual evaluated character/equipment bounds over the entire movie.
 # Blender orthographic scale is horizontal at this landscape aspect ratio.
 target=Vector((0,0,1.5));offset=Vector((3.5,-6,1.6));q=(-offset).to_track_quat('-Z','Y');right=q@Vector((1,0,0));up=q@Vector((0,1,0));bounds=[]
 bodyMeshes=[o for o in scene.objects if o.type=='MESH'and o not in sword+shield and o!=floor and not o.hide_render]
 for f in frames:
  rig.matrix_world=baseRig.copy();rig.matrix_world.translation.y+=distance(f/30)*(1 if direction=='backward'else -1);scene.frame_set(int(f),subframe=f-int(f));bpy.context.view_layer.update()
  ...
  for o in bodyMeshes+sword+shield:
   ev=o.evaluated_get(bpy.context.evaluated_depsgraph_get())
   for corner in ev.bound_box:
    p=ev.matrix_world@Vector(corner)-target;bounds.append((p.dot(right),p.dot(up)))
 minx=min(p[0]for p in bounds);maxx=max(p[0]for p in bounds);miny=min(p[1]for p in bounds);maxy=max(p[1]for p in bounds);autoTarget=target+right*((minx+maxx)/2)+up*((miny+maxy)/2);autoOffset=offset;cam.data.ortho_scale=1.12*max(maxx-minx,(maxy-miny)*900/650)
 print('FULL_SEQUENCE_CAMERA',list(autoTarget),cam.data.ortho_scale,flush=True)
try:
 for f in frames:
  rig.matrix_world=baseRig.copy();rig.matrix_world.translation.y+=distance(f/30)*(1 if direction=='backward'else -1)
  ...
  cam.location=(5,-3,2.7)if clip=='dodge'else(3.5,-6,2.8);target=Vector((0,-1.1,.9))if clip=='dodge'else Vector((0,0,1.2));cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler()
  
  if autoTarget is not None:cam.location=autoTarget+autoOffset;cam.rotation_euler=(-autoOffset).to_track_quat('-Z','Y').to_euler()
  scene.render.filepath=temporary.name;bpy.ops.render.render(write_still=True);encoder.stdin.write(open(temporary.name,'rb').read())
finally:
 try:encoder.stdin.close()
 except BrokenPipeError:pass
 code=encoder.wait()
 if os.path.exists(temporary.name):os.unlink(temporary.name)
assert code==0,code;print('ENCODED',out,clip,flush=True)
