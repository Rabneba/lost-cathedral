...
import json,array
sourceDir=ROOT+'/'+os.environ.get('PLAYER_KNEE_PROOF_DIR','docs/player-knee-prototype-proof')
manifest=json.load(open(sourceDir+'/manifest.json'))
...
scene.render.resolution_x=700;scene.render.resolution_y=750;scene.cycles.samples=8;floor.location.z=0
objects=[]
for source in manifest['meshes']:
 mesh=bpy.data.meshes.new('Actual runtime skin')
 mesh.from_pydata([(0,0,0)]*source['count'],[],[source['indices'][i:i+3]for i in range(0,len(source['indices']),3)])
 mesh.update();uv=mesh.uv_layers.new(name='UVMap')
 for face in mesh.polygons:
  face.use_smooth=True
  for loopindex in face.loop_indices:
   vertex=mesh.loops[loopindex].vertex_index;u,v=source['uv'][vertex*2:vertex*2+2];uv.data[loopindex].uv=(u,1-v)
 o=bpy.data.objects.new('Runtime skin',mesh);scene.collection.objects.link(o);o.data.materials.append(material);objects.append(o)
for pose in manifest['frames']:
 coords=array.array('f');coords.frombytes(open(sourceDir+'/'+pose['positions'],'rb').read());offset=0
 for o,source in zip(objects,manifest['meshes']):
  count=source['count']*3;o.data.vertices.foreach_set('co',coords[offset:offset+count]);o.data.update();offset+=count
 def socket(name):
 ...
 scene.render.filepath=sourceDir+'/frame-'+str(pose['frame']).zfill(3)+'.png';bpy.ops.render.render(write_still=True)
print('CONTINUOUS TRANSITION SKIN REVIEW READY',flush=True)
