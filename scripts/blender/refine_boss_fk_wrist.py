...
rows=[];failed=[];required={'maximumSecondaryWristBend':40,'method':'small wrist correction around fixed palm, captured humerus hinge/pole retained'}
for f in range(198):
 apply(poses[f]);side='Left';W=P(B(side+'Hand'));L=(P(B(side+'HandMiddle1'))-W).normalized();F=(W-P(B(side+'ForeArm'))).normalized();originalAngle=math.degrees(L.angle(F))
 if originalAngle<=40:continue
 G=grip(side);beforeHinge=hinge_error(side);originalW=W.copy();okay=True
 for iteration in range(8):
  W=P(B(side+'Hand'));Q=M(B(side+'Hand')).to_quaternion();L=(P(B(side+'HandMiddle1'))-W).normalized();F=(W-P(B(side+'ForeArm'))).normalized();angle=L.angle(F)
  if angle<=math.radians(40.001):break
  targetL=Quaternion().slerp(F.rotation_difference(L),math.radians(39.5)/angle)@F;targetQ=L.rotation_difference(targetL)@Q;localGrip=Q.inverted()@(grip(side)-W);targetW=G-targetQ@localGrip
  if not move_arm(side,targetW-W):okay=False;break
  m=M(B(side+'Hand'));setworld(B(side+'Hand'),Matrix.LocRotScale(m.translation,targetQ,m.to_scale()))
 if not okay:failed.append({'frame':f,'reason':'secondary wrist target reach'});continue
 W=P(B(side+'Hand'));L=(P(B(side+'HandMiddle1'))-W).normalized();F=(W-P(B(side+'ForeArm'))).normalized();poses[f]={p.name:p.matrix_basis.copy()for p in bones};rows.append({'frame':f,'wristBendBefore':originalAngle,'wristBendAfter':math.degrees(L.angle(F)),'shift':(W-originalW).length,'hingeErrorChangeDegrees':math.degrees(hinge_error(side)-beforeHinge),'palmResidual':(grip(side)-G).length})
