...
for i in range(74*8,94*8+1):
 ...
 prev=r
result={'fps':240,'sourceFrames':[74,94],'maxHandStep':max(rows,key=lambda r:r['handStep']),'maxForearmStep':max(rows,key=lambda r:r['forearmStepDegrees'])};json.dump(result,open(os.path.join(ROOT,'docs/combat-revision/boss-regrasp-'+TAG+'.json'),'w'),indent=2);print('REGRASP_SUBFRAMES',result,flush=True)
