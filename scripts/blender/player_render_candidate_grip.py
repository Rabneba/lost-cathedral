...
import bpy, os, sys, math, json, statistics, numpy as np
from mathutils import Vector, Matrix, Quaternion
ROOT=os.path.abspath(os.path.join(os.path.dirname(__file__),'../..'))
CANDIDATE=False
args=sys.argv[sys.argv.index('--')+1:] if '--'in sys.argv else []
asset=args[0] if args else 'assets/player-combat-revision/player-video-candidate.blend'
clip=args[1] if len(args)>1 else 'heavy';frame=int(args[2])if len(args)>2 else 28
OUT=ROOT+'/'+(args[3]if len(args)>3 else 'docs/player-combat-revision/candidate-heavy-grip')
os.makedirs(OUT,exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=ROOT+'/'+asset)
