...
bpy.ops.object.camera_add();cam=bpy.context.object;scene.camera=cam;cam.data.type='ORTHO';cam.data.ortho_scale=4.0
...
travel=json.load(open(ROOT+'/docs/player-combat-revision/grounded-trial-report.json'))[clip]['rows'];baseRig=rig.matrix_world.copy()
for f in frames:
 rig.matrix_world=baseRig.copy();xy=travel[min(len(travel)-1,round(f))]['inferredTravelBlenderXY'];rig.matrix_world.translation+=Vector((xy[0],xy[1],0))
 ...
 cam.location=(3.8,-7,2.9);target=Vector((0,-1,1));cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler()
