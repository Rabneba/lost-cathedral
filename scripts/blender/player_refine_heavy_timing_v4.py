...
def desired(name,t):
 ...
 angles=[.5,.10,-.30,-.55,-.25,.40,1.25,1.85,1.70,1.10,.45,-.30,-.70,-.70,-.70,-.70,-.65,.05,.35]
...
for name in ['heavy-v2']:
 ...
 thetas=np.unwrap(thetas);kernel=np.exp(-np.arange(-8,9,dtype=float)**2/(2*3.6**2));kernel/=kernel.sum();thetas=np.convolve(np.pad(thetas,(8,8),mode='edge'),kernel,mode='valid')
 for f in range(end+1):
...
bpy.ops.wm.save_as_mainfile(filepath=OUT+'/player-video-weapon-v4-trial.blend');json.dump(report,open(R+'/docs/player-combat-revision/weapon-orientation-v4-trial-report.json','w'),indent=2)
