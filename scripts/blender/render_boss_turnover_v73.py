...
import sys,os
ROOT=os.path.abspath(os.path.join(os.path.dirname(__file__),'../..'));sys.argv=['diagnostic','--','assets/combat-revision/boss/v73-slam-turnover-review/boss-combat-candidate.blend','slam','docs/combat-revision/fk-baseline-v67','assets/idle-video-motion/scythe-fitted.glb','2.85','1']
...
for f in [18,165]:
 ...
 for label,location in [('front',(2,-5,2.6)),('side',(5,.2,2.6))]:
  camera.location=location;target=Vector((0,-.1,1.65));camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.ortho_scale=2.8;scene.render.filepath=os.path.join(ROOT,'docs/combat-revision/boss-turnover-v73-%s-%s.png'%(f,label));bpy.ops.render.render(write_still=True)
