"""Sweep derivative: actual boot support, captured knee planes, unchanged upper body.
...
TAG=os.environ.get('VESPER_SWEEP_TAG','sweep-capture-close4-support')
...
prefix=open(ROOT+'/scripts/blender/build_boss_sweep_capture_foot_reach.py').read().split('rows=[]')[0]
...
yaws={s:{f:yaw_at(s,f)for f in [0,20,30,35,70,80,84,130,150,180,197]}for s in ['Left','Right']}
def entry(f,p,yaw):return(f,Vector((p[0],p[1],flat['Left']['height'])),yaw)
paths={
 'Left':[(0,idle['Left'],yaws['Left'][0]),(16,Vector((-.10,.42,0)),yaws['Left'][20]),(40,Vector((-.10,.42,0)),yaws['Left'][20]),(62,Vector((.42,-.49,0)),yaws['Left'][80]),(108,Vector((.42,-.49,0)),yaws['Left'][80]),(132,Vector((.15,.12,0)),yaws['Left'][130]),(164,Vector((.15,.12,0)),yaws['Left'][130]),(181,idle['Left'],yaws['Left'][197]),(197,idle['Left'],yaws['Left'][197])],
 'Right':[(0,idle['Right'],yaws['Right'][0]),(16,idle['Right'],yaws['Right'][0]),(32,Vector((-.18,-.40,0)),yaws['Right'][30]),(62,Vector((-.18,-.40,0)),yaws['Right'][30]),(74,Vector((-.42,-.02,0)),yaws['Right'][80]),(132,Vector((-.42,-.02,0)),yaws['Right'][80]),(150,Vector((-.14,-.29,0)),yaws['Right'][150]),(181,Vector((-.14,-.29,0)),yaws['Right'][150]),(197,idle['Right'],yaws['Right'][197])]
}
swings={'Left':[(0,16,.09),(40,62,.13),(108,132,.09),(164,181,.07)],'Right':[(16,32,.08),(62,74,.07),(132,150,.07),(181,197,.07)]}
# Aim is already baked into the source before leg support. Each landing target
# is rotated by the aim at landing, then held fixed through its support interval.
# Repeated identical source targets share the same world point: no skating ramp.
bodyAim=float(os.environ.get('VESPER_SWEEP_BODY_AIM','0'))
def aim_at(f):
 def cubic(a,b):x=max(0,min(1,(f-a)/(b-a)));return x*x*(3-2*x)
 return math.radians(bodyAim*cubic(0,32)*(1-cubic(108,181)))
if bodyAim:
 for side,points in paths.items():
  converted=[];last_original=None;last_world=None
  for f,p,y in points:
   if last_original is not None and (p-last_original).length<1e-6:world=last_world.copy()
   else:world=Quaternion(Z,aim_at(f))@p
   converted.append((f,world,y));last_original=p.copy();last_world=world.copy()
  paths[side]=converted
def target_at(s,f):
...
report={'source':sourceTag,'status':'legs-only review after baked wholebody aim; no runtime aim should be added','bodyAimDegreesAlreadyBaked':bodyAim,'allowedChangedBones':sorted(allowed),'maximumUnchangedWorldMatrixElementError':max_upper,'calibration':'accepted idle world Foot quaternion and ToeBase local matrix; source heading yaw; flat heel and toe contact, timed alternating swing arcs','steps':swings,'leadFoot':'Left; planted source62–108 including entire close-cut contact interval','failures':failures,'rows':rows}
