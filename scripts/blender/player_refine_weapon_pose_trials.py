...
def desired(name,t):
 ...
 times=[0,.25,.5,.75,1,1.25,1.5,1.75,2,2.25,2.5,2.75,3,3.75,4,4.5,5.13]
 angles=[.5,.10,-.30,-.55,-.6,-.35,0,2.15,1.95,1.57,-.78,-.72,-.70,-.70,-.65,.05,.35]
 angle=float(np.interp(t,times,angles));return Vector((-.08,-math.cos(angle),math.sin(angle))).normalized()
report={}
for name in ['guard-v2','light-v3','heavy-v2','hit-v3']:
 ...
 for f in range(end+1):
  ...
  theta=math.atan2(axis.dot(sp.cross(tp)),sp.dot(tp)) if min(sp.length,tp.length)>.02 else 0
...
bpy.ops.wm.save_as_mainfile(filepath=OUT+'/player-video-weapon-v2-trial.blend');json.dump(report,open(R+'/docs/player-combat-revision/weapon-orientation-v2-trial-report.json','w'),indent=2)
