...
bpy.ops.wm.open_mainfile(filepath=ROOT+'/assets/player-essential-motion/player-material-support-study.blend')
...
sources={tr.name:tr.strips[0].action.copy()for tr in rig.animation_data.nla_tracks if tr.name.startswith('run-')}
tracks={tr.name:tr for tr in rig.animation_data.nla_tracks};allReports={}
for name in sources:
 if name=='run-forward':continue
 ...
 action=sources[sourceName];endframe=round(targetAction.frame_range[1]);half=endframe/2;period=half/30;duration=endframe/30;support=period/3;speed=meta['clips'][name]['sourceSpeed']
 ...
 eligible=[x for x in candidates if x[0]] or candidates;flatTime=min(eligible,key=lambda x:x[1])[2];sourceStart=flatTime-sourcePeriod/6
 ...
 for f in range(endframe+1):
  ...
  raw.append(p)
 samples=[];report=[];drop=.10
 for f in range(endframe+1):
  phase=((f/half+2/3)%1)*period;restore(raw[f]);hip=wm(B('Hips'));hip.translation.z-=drop;setworld(B('Hips'),hip);data={}
  for side,offset in [('Right',0),('Left',period/2)]:
   ...
   if stance:travel=speed*(u-support/2);lift=0;pitch=0
    ...
    lift=.12*math.sin(math.pi*v)**1.4;pitch=math.radians(14)*math.sin(2*math.pi*v)*math.sin(math.pi*v)
...
bpy.ops.wm.save_as_mainfile(filepath=ROOT+'/assets/player-essential-motion/player-all-run-support-study.blend')
bpy.ops.export_scene.gltf(filepath=ROOT+'/assets/player-essential-motion/player-all-run-support-study.glb',export_format='GLB',use_selection=True,export_animations=True,export_animation_mode='NLA_TRACKS',export_nla_strips=True,export_frame_range=False,export_force_sampling=True,export_def_bones=False)
json.dump(allReports,open(ROOT+'/docs/player-all-run-support-study.json','w'),indent=2)
print('EIGHT RUN SUPPORT DERIVATIVE READY',flush=True)
