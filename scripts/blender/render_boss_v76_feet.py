...
import sys,os
ROOT=os.path.abspath(os.path.join(os.path.dirname(__file__),'../..'));sys.argv=['diagnostic','--','assets/combat-revision/boss/v76-slam-foot-support/boss-combat-candidate.blend','slam','docs/combat-revision/fk-baseline-v67','assets/idle-video-motion/scythe-fitted.glb','2.85','1']
...
for f in [84,182]:
 ...
 for label,location in [('front',(2,-5,.8)),('side',(5,.2,.8))]:
  camera.location=location;target=Vector((0,-.05,.42));camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.ortho_scale=1.6;scene.render.filepath=os.path.join(ROOT,'docs/combat-revision/boss-v76-feet-%s-%s.png'%(f,label));bpy.ops.render.render(write_still=True)
