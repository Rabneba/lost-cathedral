...
for name in ['light','heavy','death']:
 ...
 a=bpy.data.actions.new('video-v2-'+name);rig.animation_data.action=a;previous={}
...
bpy.ops.object.select_all(action='DESELECT');rig.select_set(True);body.select_set(True);bpy.context.view_layer.objects.active=rig;dest=O+'/player-video-candidate-v2';bpy.ops.wm.save_as_mainfile(filepath=dest+'.blend')
...
m=json.load(open(R+'/docs/player-combat-revision/player-video-candidate-manifest.json'));m['asset']='assets/player-combat-revision/player-video-candidate-v2.glb';m['sha256']=hashlib.sha256(open(dest+'.glb','rb').read()).hexdigest();m['previousCandidateSha256']='82c333ac7abfc98d596306249d5f6f5119958d8e8b0235128c221d0fb3718b0c';m['clips']['light'].update(windup=.46,active=.26,recovery=.78,hitWindows=[[.49,.68]]);m['clips']['death']['source']='Retained death body with captured new-guard entry and physical sword-floor cleanup';m['reviewStatus']='v2 candidate; fixes incorrect light blade direction, heavy floor, forearm angle-wrap and death entry; runtime/visual review pending';json.dump(m,open(R+'/docs/player-combat-revision/player-video-candidate-v2-manifest.json','w'),indent=2);json.dump(reports,open(R+'/docs/player-combat-revision/attack-v2-floor-correction-report.json','w'),indent=2)
