...
with bpy.data.libraries.load(O+'/player-video-weapon-v4-trial.blend',link=False)as(src,dst):dst.actions=wanted
...
for name in ['heavy']:
 ...
 a=bpy.data.actions.new('video-v4-'+name);rig.animation_data.action=a;previous={}
...
bpy.ops.object.select_all(action='DESELECT');rig.select_set(True);body.select_set(True);bpy.context.view_layer.objects.active=rig;dest=O+'/player-video-candidate-v4';bpy.ops.wm.save_as_mainfile(filepath=dest+'.blend')
...
m=json.load(open(R+'/docs/player-combat-revision/player-video-candidate-v2-manifest.json'));m['asset']='assets/player-combat-revision/player-video-candidate-v4.glb';m['sha256']=hashlib.sha256(open(dest+'.glb','rb').read()).hexdigest();m['previousCandidateSha256']='7832539766fbf2d9e2429d1602e8921528e25803dd128fcf2b85a2b02763a3fb';m['clips']['light'].update(windup=.46,active=.26,recovery=.78,hitWindows=[[.49,.68]]);m['clips']['death']['source']='Retained death body with captured new-guard entry and physical sword-floor cleanup';m['reviewStatus']='v4 candidate; heavy blade descent synchronized to arm drop and temporally regularized; runtime/visual review pending';json.dump(m,open(R+'/docs/player-combat-revision/player-video-candidate-v4-manifest.json','w'),indent=2);json.dump(reports,open(R+'/docs/player-combat-revision/attack-v4-floor-correction-report.json','w'),indent=2)
