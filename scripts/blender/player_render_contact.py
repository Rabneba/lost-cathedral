...
for f in frames:
 rig.matrix_world=rigBase.copy()
 scene.frame_set(int(f),subframe=f-int(f));bpy.context.view_layer.update()
 t=f/30
 if clip=='dodge':
  u=max(0,min(1,(t-.12)/.48));travel=1.2*(1-(1-u)**2);centerY=.6
 elif clip.startswith('run-'):
  travel=-meta['clips'][clip]['sourceSpeed']*t;centerY=-meta['clips'][clip]['sourceSpeed']*meta['clips'][clip]['duration']/2
 else:travel=0;centerY=0
 ...
 cam.location=(5,centerY,2.0);target=Vector((0,centerY,1.0));cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=6.2 if clip.startswith('run-') else 4.2
