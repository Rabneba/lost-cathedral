...
ROOT=os.path.abspath(os.path.join(os.path.dirname(__file__),'../..'));bpy.ops.wm.open_mainfile(filepath=os.path.join(ROOT,'assets/combat-revision/boss/v62-slam-stable-quaternion-branch/boss-combat-candidate.blend'));rig=next(o for o in bpy.context.scene.objects if o.type=='ARMATURE');rig.animation_data.action=None
...
source=os.environ.get('VESPER_UPPERARM_SOURCE')=='1'
if source:
 bpy.ops.wm.open_mainfile(filepath=os.path.join(ROOT,'assets/boss-essential-motion/boss-sources.blend'));rig=next(o for o in bpy.context.scene.objects if o.type=='ARMATURE');rig.animation_data.action=None
 for t in rig.animation_data.nla_tracks:t.mute=t.name!='source-slam'
rows=[]
for f in range(60,101):
 ...
 rows.append(r)
json.dump(rows,open(os.path.join(ROOT,'docs/combat-revision/boss-upperarm-raw.json'if source else'boss-upperarm-v62.json'),'w'),indent=2)
for s in['Left','Right']:
