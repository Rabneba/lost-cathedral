...
OUT=os.path.join(ROOT,'docs/combat-revision/boss-target-search-v6.json')
...
rng=np.random.default_rng(44061);out={'input':PATH,'rearAnchorActive':.04,'maximumWristBend':32,'shaftClearanceRequired':.012,'method':'exact palm/elbow circle solution; current evaluated torso and hood BVH; candidate positions only, not a baked animation','frames':{}}
...
for clip,f in queries:
 ...
 for i in range(16001):
  if i==0:C=C0.copy();A=A0.copy();spacing=report['spacing'];Q=Q0.copy()
   ...
   Q=delta@Q0;A=np.array(Q@Vector((0,0,1)));spacing=float(rng.uniform(.20,.50))
  solved={}
