// Write the reviewed V17 manifest: V15's manifest with only the light and heavy entries replaced
...
//
//   node scripts/finalize-v17-manifest.mjs
import {readFileSync, writeFileSync} from 'node:fs';
import {createHash as hash} from 'node:crypto';

const BASE = 'docs/player-motion-revision-2/player-video-candidate-v15-manifest.json';
const GLB = 'assets/player-motion-revision-3/player-video-candidate-v17.glb';
const OUT = 'docs/player-motion-revision-3/player-video-candidate-v17-manifest.json';
const ASSET_OUT = 'assets/player-motion-revision-3/player-video-candidate-v17-manifest.json';
...
const MEASURED = {
  light: {
    ...
    note: 'A lateral cut: the blade cocks back beside the head, sweeps horizontally through the opponent and follows through low across the body.',
  },
  heavy: {
    ...
    note: 'A committed diagonal chop: the blade is cocked high behind the head, driven forward through the opponent and down across the body to a low finish.',
  },
};
const TRANSFER = 'user-supplied "Sword 4" pack, retargeted to this rig by '
  ...
  + 'off the wrist, both grips held at the V15 guard pose, the shield arm braced back 25 deg and down '
  + '10 deg out of the cut and returned to guard, hips travel kept, soles grounded through skinned '
  + 'foot vertices, sockets hand-relative';

const meta = JSON.parse(readFileSync(BASE, 'utf8'));
const sha = hash('sha256').update(readFileSync(GLB)).digest('hex');
meta.asset = GLB;
meta.revision = 'sword4-attacks-braced-2026-09-16';
meta.previousCandidateSha256 = meta.sha256;
meta.sha256 = sha;
meta.reviewStatus = 'V17: light and heavy replaced by retargeted clips from the user\'s Sword 4 pack '
  + '(library clips), with the shield arm braced out of the cut so the blade never passes through the '
  + 'body, the shield or the left arm (0 self-intersecting frames, against 16 and 47 in V15). Every '
  + 'other clip is byte-identical to V15. User judgement pending.';
...
meta.limitations = [
  ...
  'The Sword 4 pack is sword-only: the light and heavy hold both hand grips and the shield arm at the '
  + 'V15 guard pose rather than transferring them, so the shield never swings through the body; the '
  + 'shield arm is then braced back and down out of the sword path and returned to guard, which is '
  + 'authored in the transfer script (VESPER_SHIELD_PULL_DEG / _DROP_DEG / _SPAN), not captured.',
