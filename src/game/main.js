...
import * as T from 'three';import{createRendering}from'./rendering.js';import{buildArena}from'./arena.js';import{loadActors}from'./actors.js';import{ASSETS,MUSIC_TRACKS}from'./asset-paths.js';import{Fight,HEALTH}from'./encounter.js';import{MotionState}from'./motion.js';import{sweptWeaponContact,strikeIndex}from'./weapon-motion.js';import{GameAudio}from'./audio.js';import{Effects}from'./effects.js';import{createCombatEffects}from'./impact-effects.js';import{startCaptureWorker}from'./capture-worker.js';
const $=id=>document.getElementById(id),canvas=$('game');const graphics=createRendering(canvas),{scene,camera,renderer}=graphics;const fight=new Fight(ASSETS.motion),motion=new MotionState({animationMetadata:ASSETS.motion,bounds:{minX:-9.5,maxX:9.5,minZ:-15.5,maxZ:16},approachDistance:ASSETS.motion?.boss?.approachDistance??2.35});const audio=new GameAudio(ASSETS),effects=new Effects(scene);let floorStyle='basalt',actors,arena,combat,paused=false,ready=false,lock=false,yaw=0,pitch=.15,distance=5.2,shake=0,fovKick=0,accum=0,last=performance.now(),elapsed=0,resultAt=0,noticeUntil=0,bufferedAction=null,hitStop=0;const keys=new Set(),frames=[],attackInput=new AttackInput();let lastStat=0,frameCount=0,lastRenderAt=0;
const paths=ASSETS,dimensions={player:actorDimensions(false,ASSETS.motion.player),boss:actorDimensions(true,ASSETS.motion.boss)};
try{$('load-detail').textContent='Carving the cathedral.';arena=await buildArena(scene,{stone:{map:ASSETS.stone},floor:{map:ASSETS.floor}});motion.groundColliders=arena.groundColliders;arena.setFloorStyle(floorStyle);$('load-detail').textContent='The exile and the keeper.';actors=await loadActors(scene,paths);for(const a of Object.values(actors))a.gizmo.visible=!$('telemetry').hidden;actors.player.reset(motion.player);actors.boss.reset(motion.boss);actors.player.update(.2,0,fight.player,motion.player);actors.boss.update(.2,0,fight.boss,motion.boss);combat=createCombatEffects({scene,camera,effects,actors,fight,audio,post:graphics.post});graphics.warm();ready=true;$('loading').hidden=true;}catch(e){console.error(e);$('load-detail').textContent='An asset failed to load. Reload to try again.';}
function input(){return{forward:(keys.has('KeyW')?1:0)-(keys.has('KeyS')?1:0),right:(keys.has('KeyD')?1:0)-(keys.has('KeyA')?1:0),cameraYaw:yaw,lockOn:lock};}
...
function start(){if(!ready)return;clearPointerGesture();fight.reset();motion.reset();fight.start();keys.clear();paused=false;lock=true;yaw=0;pitch=.15;resultAt=0;effects.reset();combat?.reset();graphics.puddles.reset();bufferedAction=null;hitStop=0;shake=0;fovKick=0;camera.fov=52;camera.updateProjectionMatrix();attackInput.cancel();audio.clear();audio.start();liftVeil();$('result').hidden=true;$('hud').hidden=false;actors.player.reset(motion.player);actors.boss.reset(motion.boss);actors.player.update(.2,elapsed,fight.player,motion.player);actors.boss.update(.2,elapsed,fight.boss,motion.boss);accum=0;last=performance.now();$('phase-caption').textContent='THE VIGIL';canvas.focus();}
$('begin').onclick=start;$('retry').onclick=start;$('restart').onclick=()=>{closeSettings();start();};
function openSettings(){paused=true;keys.clear();clearPointerGesture();audio.pause();$('settings-title').textContent=fight.status==='ready'?'Settings':'Pause';$('settings').showModal();if(document.pointerLockElement)document.exitPointerLock();}
...
function unlockAudio(e){if(e.code==='Escape'||!audio.enabled)return;removeEventListener('keydown',unlockAudio);removeEventListener('pointerdown',unlockAudio);if(!audio.suspended)audio.resume();}
...
addEventListener('pointercancel',()=>{clearPointerGesture();attackInput.cancel();keys.delete('Block');});
...
addEventListener('blur',()=>{keys.clear();clearPointerGesture();attackInput.cancel();if(fight.status==='fighting'&&!paused&&!workerMode)openSettings();});
...
function handleEvents(){for(const e of fight.drain()){
 ...
 if(e.type==='guard-break')notice('Guard broken · Lower your shield to recover');if(e.type==='heal'){effects.burst(new T.Vector3(motion.player.x,1.1,motion.player.z),0xeeb96b,32);notice('Ember restored');}if(e.type==='phase-change'){notice('The last vow is broken');$('phase-caption').textContent='THE REAPING';effects.burst(new T.Vector3(motion.boss.x,1.3*dimensions.boss.scale,motion.boss.z),0x8dafc0,50);}
