// Reconstruct foot trajectories only while locomotion poses crossfade.
...
// Build a transform-only reference once. No mesh, texture or skeleton copies
// are retained, and playback never evaluates an additional animation rig.
export function cacheFeet(root,clips,hz=120){
  ...
  if(feet.some(foot=>!foot))return {cache,buildMilliseconds:performance.now()-start,bytes,samples,proxyNodes:nodes,hz};
  for(const clip of clips.filter(c=>gait(c.name))){
    // Round to align the cache with exported 30 Hz keys despite float duration error.
...
export class LocomotionFootBlend extends FootPlant {
  constructor(root,soles,clips){
    super(root,soles);this.root=root;const built=cacheFeet(root,clips);this.cache=built.cache;
    this.cacheStats={bytes:built.bytes,samples:built.samples,hz:built.hz,buildMilliseconds:built.buildMilliseconds,proxyNodes:built.proxyNodes};
    this.applied=false;
  }
  apply(animation,{rawPose=false,directClipTime=false}={}){
    this.applied=false;const transition=animation.transition;
    if(rawPose||directClipTime||!transition||this.legs.length!==2)return;
    const layers=[...animation.layers].filter(([,l])=>l.weight>1e-6);
    if(layers.length<2||layers.some(([name,l])=>!gait(name)||l.oneShot||!this.cache.has(name)))return;
    // A centered 33 ms window rounds touchdown without delaying input. Ease
    // its influence at either end so entering/leaving a fade adds no jump.
    ...
    for(let side=0;side<2;side++){
      ...
      // Keep authored flight height. Positions are cached in model space;
      // current actor translation and facing are applied only at this step.
